...
# centrala + oddziały
class SwiftCodeHeadquarterResponse(SwiftCodeResponse):
...
@app.get("/v1/swift-codes/{swift_code}", response_model=SwiftCodeHeadquarterResponse | SwiftCodeResponse)
def get_swift_code(swift_code: str):
    ...
    if code.is_headquarter:
        ...
        response["branches"] = [{
            "address": f"{branch.address}, {branch.town_name}",
...
@app.get("/v1/swift-codes/country/{countryISO2code}", response_model=CountryResponse)
def get_swift_codes_by_country(countryISO2code: str):
    ...
    swift_codes = [{
        "address": f"{record.address}, {record.town_name}",
        "bankName": record.bank_name,
        "countryISO2": record.country_iso2_code,
